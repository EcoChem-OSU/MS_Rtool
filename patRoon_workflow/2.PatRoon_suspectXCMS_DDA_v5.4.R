...
if (MD.filter=="YES") # Mass defect filtration
  ...
  fGroupsF <- patRoon::delete(fGroups, j = function(pl, grp, ana, t)
  {
    # NOTE: get the neutralMass from the MS spectrum
    mz <- fGroups@features@features$mz 
    MD <- mz - round2(mz,0)
    return(MD<MD.minmax[1]|MD>MD.minmax[2])
  })
